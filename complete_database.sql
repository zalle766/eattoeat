-- =====================================================
-- قاعدة بيانات شاملة لتطبيق Eat to Eat
-- =====================================================
...
CREATE DATABASE `eat_to_eat` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- عرض المطاعم مع معلومات المالك
CREATE VIEW `restaurants_with_owner` AS
SELECT 
    r.*,
...
LEFT JOIN users c ON cou.user_id = c.id;

-- عرض المنتجات مع معلومات المطعم
CREATE VIEW `products_with_restaurant` AS
SELECT 
    p.*,
    r.name as restaurant_name,
...
WHERE p.is_available = 1 AND r.is_active = 1;

-- =====================================================
-- إنشاء الإجراءات المخزنة
-- =====================================================

-- إجراء لحساب متوسط التقييم
DELIMITER //
CREATE PROCEDURE UpdateRestaurantRating(IN restaurant_id INT)
BEGIN
    ...
    AND reviewable_id = restaurant_id;
...
-- إجراء لحساب متوسط تقييم السائق
DELIMITER //
CREATE PROCEDURE UpdateCourierRating(IN courier_id INT)
BEGIN
    DECLARE avg_rating DECIMAL(3,2);
    DECLARE total_deliveries INT;
    
    SELECT AVG(rating), COUNT(*) INTO avg_rating, total_deliveries
    FROM reviews 
    WHERE reviewable_type = 'App\\Models\\Courier' 
    AND reviewable_id = courier_id;
...
-- تحديث تقييم المطعم عند إضافة تقييم جديد
DELIMITER //
CREATE TRIGGER update_restaurant_rating_after_review_insert
AFTER INSERT ON reviews
FOR EACH ROW
BEGIN
    IF NEW.reviewable_type = 'App\\Models\\Restaurant' THEN
        CALL UpdateRestaurantRating(NEW.reviewable_id);
    END IF;
END //
DELIMITER ;

-- تحديث تقييم السائق عند إضافة تقييم جديد
DELIMITER //
CREATE TRIGGER update_courier_rating_after_review_insert
AFTER INSERT ON reviews
FOR EACH ROW
BEGIN
    IF NEW.reviewable_type = 'App\\Models\\Courier' THEN
        CALL UpdateCourierRating(NEW.reviewable_id);
    END IF;
END //
DELIMITER ;
...
CREATE USER 'eattoeat_user'@'localhost' IDENTIFIED BY 'eattoeat_password';
GRANT SELECT, INSERT, UPDATE, DELETE ON eattoeat.* TO 'eattoeat_user'@'localhost';

-- إنشاء مستخدم للقراءة فقط (للتقارير)
CREATE USER 'eattoeat_readonly'@'localhost' IDENTIFIED BY 'eattoeat_readonly_password';
GRANT SELECT ON eattoeat.* TO 'eattoeat_readonly'@'localhost';
